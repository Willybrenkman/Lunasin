...
function calculateBaseline(debts, strategy) {
  ...
  if (strategy === "snowball") active.sort((a, b) => a.remaining - b.remaining);
  else active.sort((a, b) => b.interest - a.interest);
...
export function getDebtHealth(debts) {
  ...
  const totalDebt = debts.reduce((s, d) => s + Number(d.total), 0);
...
export function getSmartPriority(debts) {
  return debts.map(d => {
    ...
    const sizeWeight = (Number(d.total) / 1000000) * 0.5;
